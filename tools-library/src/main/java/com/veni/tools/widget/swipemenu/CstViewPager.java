...
public class CstViewPager extends ViewPager {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        //LogUtils.iTag(TAG, "onInterceptTouchEvent() called with: ev = [" + ev + "]");
        int x = (int) ev.getX();
        int y = (int) ev.getY();
            ...
            case MotionEvent.ACTION_MOVE:
                if (isHorizontalScroll(x, y)) {
                    ...
                    if (isReactFirstPage() && isScrollRight(x)) {
                        //LogUtils.eTag(TAG, "第一页的手指向右滑]");
                        intercept = false;
                    } else if (isReachLastPage() && isScrollLeft(x)) {
                        //LogUtils.eTag(TAG, "最后一页的左滑");
                        intercept = false;
                    } else {
                        //LogUtils.eTag(TAG, "其他情况");
                        intercept = true;
                    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        //LogUtils.iTag(TAG, "onTouchEvent() called with: ev = [" + ev + "]");
